...
- Runs at **:10, :25, and :40** past the hour. GitHub can skip that schedule; the Cloudflare Worker also `workflow_dispatch`es this workflow at those same minutes when `GITHUB_DISPATCH_TOKEN` is set (see [worker/README.md](worker/README.md)). Actions stays primary; the Worker is a backup kick.
